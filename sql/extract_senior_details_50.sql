/* 
    Script:   Extract Over 50's Results
    Created:  2019-05-18
    Author:   Michael George / 2015GEOR02
   
    Purpose:  Unofficial rankings for the Over-50's - https://github.com/Logiqx/wca-ipy.
*/

-- Extract seniors
SELECT p.id AS personId, p.name AS personName, c.name AS country, IFNULL(s.username, '?') AS username
INTO OUTFILE '/home/jovyan/work/wca-ipy/data/public/extract/known_senior_details_50.csv' FIELDS TERMINATED BY ',' OPTIONALLY ENCLOSED BY '"'
FROM Persons AS p
INNER JOIN Seniors AS s ON s.personId = p.id
INNER JOIN Countries AS c ON p.countryId = c.id
WHERE p.subid = 1 AND p.year > 1900 AND p.year <= YEAR(CURDATE()) - 50
AND EXISTS
(
...
INTO OUTFILE '/home/jovyan/work/wca-ipy/data/public/extract/known_senior_averages_50.csv' FIELDS TERMINATED BY ',' OPTIONALLY ENCLOSED BY '"'
FROM
(
...
INTO OUTFILE '/home/jovyan/work/wca-ipy/data/public/extract/known_senior_singles_50.csv' FIELDS TERMINATED BY ',' OPTIONALLY ENCLOSED BY '"'
FROM
(
  ...
  FROM Results AS r
  INNER JOIN Competitions AS c ON r.competitionId = c.id
  INNER JOIN Persons AS p ON r.personId = p.id AND p.year > 1900 AND p.year <= YEAR(CURDATE()) - 50
  WHERE best > 0
  HAVING age_at_comp >= 50
) AS tmp_results
GROUP BY eventId, personId
