...
FROM
(
  SELECT DATE_ADD(UTC_DATE(), INTERVAL -10 DAY) AS cutoff_date, eventId, "average" AS result, age_category, continentId, COUNT(DISTINCT personId) AS num_seniors
  FROM
  (
...
FROM
(
  SELECT DATE_ADD(UTC_DATE(), INTERVAL -10 DAY) AS cutoff_date, eventId, "single" AS result, age_category, continentId, COUNT(DISTINCT personId) AS num_seniors
  FROM
  (
    SELECT personId, continentId, eventId,
      TIMESTAMPDIFF(YEAR, DATE_FORMAT(CONCAT(p.year, '-', p.month, '-', p.day), '%Y-%m-%d'), start_date) AS age_at_comp
