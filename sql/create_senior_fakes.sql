...
(
	...
	FROM SeniorStatsExtra AS ss
	JOIN SeniorBestsExtra AS sb ON sb.viewId = ss.viewId AND
		(sb.rowNo >= ss.minRowNo OR prevResult IS NULL) AND (sb.rowNo <= ss.maxRowNo OR nextResult IS NULL)
    WHERE stepNo IS NULL
...
(
	...
	SELECT viewId, fakeResult, groupNo, numMissing, 'FAKE_RANGE' AS fakeId
	FROM
	(
		SELECT viewId,
...
(
	...
	SELECT viewId, groupResult * groupSize - totResult AS fakeResult, groupNo, numMissing, 'FAKE_EXACT' AS fakeId
	FROM SeniorStatsExtra
	WHERE stepNo = 2
	AND numMissing = 1
	AND groupSize >= 4
