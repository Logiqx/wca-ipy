...
(
  ...
  (
    ...
    INNER JOIN wca_20190130.Persons AS p ON r.personId = p.id AND p.subid = 1 AND p.year > 0 AND p.year <= YEAR(CURDATE()) - 40
    WHERE average > 0
...
SELECT *
INTO OUTFILE '/home/jovyan/work/wca-ipy/data/private/senior_averages_delta.csv' FIELDS TERMINATED BY ',' OPTIONALLY ENCLOSED BY '"'
FROM
(
